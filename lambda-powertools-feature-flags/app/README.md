...
```bash
app$ make run
app$ curl http://localhost:3000/hello
```
...
```bash
app$ sam logs -n HelloWorldFunction --stack-name <Name-of-your-deployed-stack> --tail
```
...
```bash
app$ aws cloudformation delete-stack --stack-name app
```
...
```bash
app$ pipenv lock -r > hello_world/requirements.txt
```
