...
function ProfileLink({ user }) {
    ...
    function subscribeNotifications() {
        if (user?.signInUserSession.idToken.payload['cognito:groups']?.includes("Approvers")) {
            API.graphql(
                graphqlOperation(onVacationRequestNotification)
            ).subscribe({
                next: () => {
    ...
    return (
        <div className="-mx-2 border-t-2 py-2">
            <div className="pl-4 flex items-center pb-2">
                <img alt="profile-pic" className="w-16 h-16 rounded-full" src={profilePic} />
                <div>
                    <div className="flex">
                        <p className="pl-2 text-base text-gray-800">{user.username}</p>
                        {notificationCount > 0 &&
                            <button className="px-2 rounded-full mx-2 bg-red-600 ">
                                <p className="text-sm text-white font-semibold">{notificationCount}</p>
                            </button>
                        }
